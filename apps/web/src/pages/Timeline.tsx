              ...
              >
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  {group.items.map((item) => {
                    const serviceMatch = item.isRecurring ? matchServiceIcon(item.name) : null;
                    return (
                    <div key={item.installmentId} className="flex items-center gap-3 rounded-xl surface border border-[rgb(var(--border))] px-3 py-2.5 shadow-soft">
                      {serviceMatch ? (
                        <span
                          ...
                          style={{ backgroundColor: `${serviceMatch.color}1a` }}
                        >
                          <serviceMatch.Icon className="h-3.5 w-3.5" style={{ color: serviceMatch.color }} />
                        </span>
                      ) : (
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-muted">
                          {item.isCash ? "À vista" : item.isRecurring ? "Assinatura" : `${item.number}/${item.installmentsCount}`}
                        </p>
                      </div>
